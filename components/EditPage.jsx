...
import {
  FiEdit3, FiEye, FiSave, FiDownload,
  FiArrowLeft, FiHash, FiCode, FiLayers, FiFileText,
} from "react-icons/fi";
import { LANG_MAP } from "@/lib/languages";
import { getById, persist, downloadFile } from "@/lib/storage";
...
export default function EditPage({ id }) {
  ...
  useEffect(() => {
    if (!user) return;
    setLoading(true);
    getById(user.uid, id).then((s) => {
      if (!s) { setNotAuth(true); setLoading(false); return; }
      if (s.ownerId !== user.uid) { setNotAuth(true); setLoading(false); return; }
      setSnippet(s);
      setTitle(s.title || "");
      ...
      setNotes(s.notes || "");
      setLoading(false);
    }).catch(() => { setNotAuth(true); setLoading(false); });
  }, [id, user]);
  ...
  const save = async () => {
    ...
    await persist(user.uid, updated);
  ...
  if (notAuth) return (
    <div className="page-enter" style={{ maxWidth:600, margin:"80px auto", padding:"0 24px", textAlign:"center" }}>
      <h2 style={{ fontSize:22, fontWeight:800, marginBottom:10 }}>Not authorized</h2>
      <p style={{ color:"var(--t2)", marginBottom:24 }}>You don&apos;t have permission to edit this snippet.</p>
    ...
    <div className="page-enter page-inner" style={{ maxWidth:1000, margin:"0 auto", padding:"36px 24px 80px" }}>
      <div style={{ marginBottom:28 }}>
        ...
        <p style={{ color:"var(--t2)", fontSize:14, marginLeft:46 }}>
          Update your snippet and save changes.
        </p>
      </div>
      ...
      <div className="bottom-actions" style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginTop:20, flexWrap:"wrap", gap:12 }}>
        <button className="btn btn-ghost" onClick={() => router.push(`/snippet/${id}`)}>
        ...
        <div style={{ display:"flex", gap:10, flexWrap:"wrap" }}>
          ...
          <button className="btn btn-primary" onClick={save} style={{ fontSize:14, padding:"9px 22px" }}>
